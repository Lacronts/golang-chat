      ...
      switch (Object.keys(JSONData)[0]) {
        ...
        case EReceivedDataKey.MESSAGE: {
          ...
          if (newMessage.groupName) {
            const findedUser = activeUsers.find(user => user.name === newMessage.groupName);
            findedUser.messages = findedUser.messages.concat(newMessage);
            this.dispatch({ type: ChatActionTypes.UPDATE_ACTIVE_USERS, payload: [...activeUsers] });
          } else if (userName === newMessage.from) {
            const findedUser = activeUsers.find(user => user.name === targetUser);
            findedUser.messages = findedUser.messages.concat(newMessage);
            this.dispatch({ type: ChatActionTypes.UPDATE_ACTIVE_USERS, payload: [...activeUsers] });
          } else {
            const findedUser = activeUsers.find(user => user.name === newMessage.from);
            findedUser.messages = findedUser.messages.concat(newMessage);
            this.dispatch({ type: ChatActionTypes.UPDATE_ACTIVE_USERS, payload: [...activeUsers] });
          }
